...
public class Repository {
     ...
     */
    private final Lazy<Commit> HEADCommit =  new Lazy(()->getBranchHeadCommit(currentBranch.get()));
    /**
    ...
    private final Lazy<StagingArea> stagingArea=lazy(()->{
       ...
       StagingArea.fromFile():new StagingArea();
       s.setTracked(HEADCommit.get().getTracked());
       return s;
    });

    /**
     * Exit if the repository at current working directory is not initialized.
     */
    ...
    /**
     * Initialized a repository ar the current working directory.
     *
     * <pre>
     *    .gitlet
     *      * ├── HEAD
    ...
    public static void init(){
        if(GITLET_DIR.exists()){
            exit("A gitlet version-control system already exsits in the current directory.");}
            mkdir(GITLET_DIR);
            mkdir(REFS_DIR);
    ...
    public static void globalLog() {
        ...
        //As the project spec goes,the runtime should be O(N).
    ...
    /**
     * Get the whole commit id.Exit with message if it doesn't exit.
     *
    ...
    private static String getActualCommitId(String commitId){
        ...
        if(!objectDir.exists()){
            exit("No commit with that id exits.");
        }
        ...
        if(!isFound) exit("No commit with that id exits.");
        else{
        ...
        return commitId;}
    /**
     * Get the id of the latest common ancestors of the two commits.
     *
     ...
     */
    @SuppressWarnings("ConstantConditions")
    private static Commit getLatestCommonAncestorsCommit(Commit commitA,Commit commitB){
        Comparator<Commit> commitComparator =Comparator.comparing(Commit::getDate).reversed();
        ...
        Set<String> checkedCommitIds=new HashSet<>();
        while(true){
        Commit lastCommit =commitsQueue.poll();
        List<String>parentCommitIds=lastCommit.getParents();
    ...
    /**
     * Checkout file from specific commit id.May be not in HEAD commit
     *
     * @param commitId Commit SHA1 id
     * @param fileName Name of the file
     */
    ...
    public void rmBranch(String targetBranchName){
        File targetBranchHeadFile =getBranchHeadFile(targetBranchName);
        if(!targetBranchHeadFile.exists()){
            exit("A branch which is not exist");}
        if(targetBranchName.equals(currentBranch.get())){
                exit("Can't remove the current branch.");
            }
    ...
    public void merge(String targetBranchName) {
        ...
        Commit lcaCommit = getLatestCommonAncestorsCommit(HEADCommit.get(), targetBranchHeadCommit);
        ...
        for (Map.Entry<String, String> entry : lcaCommitTrackedFilesMap.entrySet()) {
            ...
            if (targetBranchHeadCommitBlobId != null) { // exists in the target branch
                ...
                // case 2, case 7
            } else {//delete in the target branch
                if(HEADCommitBlobId !=null){//exists in the current branch
